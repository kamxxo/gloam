module.exports = {
  ...
  execute(client, connection, config, message, args) {
    ...
    connection.query(
      `INSERT INTO Punishments (type, reason, punishmentTo, punishmentFrom) VALUES ("Ban", "${reason}", ${user.id}, ${message.author.id})`,
      (err, result) => {
        if (err)
          return client.helpers
            .get("MYSQLErrorHandler")
            .InsertError(message.channel, err, client);
        let bannedEmbed = client.helpers
          .get("CreateEmptyEmbed")
          .execute("Warning", client, false)
          .setTitle(`${user.username} banned`)
          .setThumbnail(user.avatarURL)
          .setDescription(
            `${message.author.username} banned ${user.username} for \`\`${reason}\`\``
          );
        message.channel.send(bannedEmbed).then(m => {
          setTimeout(() => {
            m.delete();
          }, 5000);
        });
        ...
        user
          .send(
            client.helpers
              ...
              .setDescription(
                `You've been banned for \`\`${reason}\`\` by ${message.author.username}`
              )
          )
          .then(m => {
